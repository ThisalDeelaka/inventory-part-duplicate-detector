...
- Channel top-k ordering is score descending after self exclusion; candidates
  with exactly equal authoritative full-precision retrieval similarity are
  ordered and selected by immutable `record_ref_key` ascending. Display or
  persisted score rounding occurs only after membership selection. Input
  position and source-row ordinal are not retrieval tie breakers.
