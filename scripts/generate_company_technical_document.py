...
DOCS = ROOT / "docs"
MD_PATH = DOCS / "Part_Master_Duplication_Identifier_Technical_Architecture.md"
DOCX_PATH = DOCS / "Part_Master_Duplication_Identifier_Technical_Architecture.docx"
...
def add_markdown_to_docx(doc: Document, markdown: str) -> None:
    for raw_line in markdown.splitlines():
        ...
        if line.startswith("# "):
            ...
            # Tables remain readable as preformatted text in the Word version.
            p = doc.add_paragraph()
            run = p.add_run(line)
            ...
            continue
        elif line.startswith("    ") or line.startswith("      ") or line in {"User Browser", "React + Vite Frontend", "FastAPI Backend", "SQLite Demo Database"}:
            p = doc.add_paragraph()
            run = p.add_run(line)
            run.font.name = "Courier New"
            run.font.size = Pt(9)
...
def build_docx() -> None:
    ...
    add_markdown_to_docx(doc, DOCUMENT)
